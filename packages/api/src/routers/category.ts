...
import {
  ...
  FreelancerLevel,
  DeliveryTime,
} from "@Alpha/db/prisma/generated/client"; // Import all necessary enums
...
export const categoryRouter = router({
  ...
  getExperienceLevels: publicProcedure.query(
    (): { experienceLevels: { id: string; name: string }[] } => {
      const experienceLevels: { id: string; name: string }[] = Object.values(
        ExperienceLevel
      ).map((level) => ({
        id: level as string,
        name: (level as string).replace(/_/g, " "),
      }));
  ...
  getFreelancerLevels: publicProcedure.query(
    (): { freelancerLevels: { id: string; name: string }[] } => {
      const freelancerLevels: { id: string; name: string }[] = Object.values(
        FreelancerLevel
      ).map((level) => ({
        id: level as string,
        name: (level as string).replace(/_/g, " "),
  ...
  getDeliveryTimes: publicProcedure.query(
    (): { deliveryTimes: { id: string; name: string }[] } => {
      const deliveryTimes: { id: string; name: string }[] = Object.values(
        DeliveryTime
      ).map((time) => ({
        id: time as string,
        name: (time as string).replace(/_/g, " "),
      }));
